...
namespace Faunus {
  ...
  namespace Tabulate {
    ...
    /*  Tabulator with logarithmic search, work not so well for intel compiler
    ...
    struct tabulator : public tabulatorbase<T> {
      ...
      generate(std::function<T(T)> &f) {
        ...
        for (i=0; i < mngrid; i++) {
          ...
          for (unsigned int k = 0; k < ubuft.size(); k++)
     ...
     */
    template<typename T=double>
    struct tabulatorintel : public tabulatorbase<T> {
      typedef tabulatorbase<T> base; // for convenience, only
